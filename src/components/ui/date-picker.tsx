...
const TimeSegment = ({ segment, state }: TimeSegmentProps) => {
  ...
  return (
    <div
      ...
      className={cn(
        // base
        "relative block w-full appearance-none rounded-small border px-2.5 py-1.5 text-left uppercase tabular-nums shadow-sm outline-none sm:text-sm",
        // border color
...
const triggerStyles = tv({
  base: [
    // base
    "peer flex w-full cursor-pointer appearance-none items-center gap-x-2 truncate rounded-small border-[2px] px-3 py-2 shadow-sm outline-none transition-all sm:text-sm",
    // background color
    "bg-white dark:bg-neutral-950 ",
    // border color
    "border-default-200 dark:border-default-200 hover:border-default-400 dark:hover:border-default-400",
    // text color
    "text-neutral-900 dark:text-neutral-50",
    // placeholder color
    ...
    "disabled:dark:border-neutral-800 disabled:dark:bg-neutral-800 disabled:dark:text-neutral-500",
    // focus
    focusInput,
    // invalid (optional)
    ...
    return (
      <PopoverPrimitives.Trigger asChild>
        <button
          ...
          {...props}
        >
          <CalendarIcon className="size-5 shrink-0 text-neutral-400 dark:text-neutral-600" />
  ...
  return (
    <PopoverPrimitives.Portal>
      <PopoverPrimitives.Content
        ...
        className={cn(
          // base
          "relative z-50 w-fit rounded-small border text-sm shadow-xl shadow-black/[2.5%]",
          // widths
...
const PresetContainer = <TPreset extends Preset, TValue>({
  ...
  const handleClick = (preset: TPreset | undefined) => {
    if (!preset) return onSelect(undefined);
  ...
  return (
    <ul role="list" className="flex items-start gap-x-2 gap-y-1 sm:flex-col">
      {/* add an all timeoption when clicked basicaly sets the date to undeifned */}

      <li key={"all-time"} className="sm:w-full sm:py-px">
        <button
          title={"Tous le temps"}
          className={cn(
            // base
            ...
            {
              "bg-primary text-primary-foreground hover:bg-primary/90 focus-visible:bg-primary/90 dark:bg-primary dark:hover:bg-primary/90":
                !currentValue,
            },
          )}
          onClick={() => handleClick(undefined)}
          aria-label={`Select tous le temps`}
        >
          <span>Tous le temps</span>
        </button>
      </li>
        ...
        <div className="flex">
          <div className="flex flex-col sm:flex-row sm:items-start">
            {presets && presets.length > 0 && (
              <div
                className={cn(
                  ...
                  "border-b border-neutral-300 dark:border-neutral-800 sm:border-b-0 sm:border-r",
                  "overflow-auto",
                )}
              >
            ...
            <div>
              ...
              <div className="flex items-center gap-x-2 border-t border-neutral-300 p-3 dark:border-neutral-800">
                <Button
                  variant="secondary"
        ...
        <div className="flex">
          <div className="flex flex-col overflow-x-auto sm:flex-row sm:items-start">
            ...
            <div className="overflow-x-auto">
              ...
              />
              {showTimePicker && (
                <div className="flex items-center justify-evenly gap-x-3 border-t border-neutral-300 p-3 dark:border-neutral-800">
              ...
              <div className="border-t border-neutral-300 p-3 dark:border-neutral-800 sm:flex sm:items-center sm:justify-between">
                <p className="tabular-nums text-neutral-900 dark:text-neutral-50">
                  <span className="text-neutral-700 dark:text-neutral-300">
                    {translations?.range ?? "Range"}:
                  </span>{" "}
                  <span className="font-medium">
                    {displayRange ?? "Tous le temps"}
                  </span>
                </p>
